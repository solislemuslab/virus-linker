...
server = function(input, output, session){
  mydf = reactiveVal()
  mydf_num = reactiveVal()
  min_gap = 0.01

  observeEvent(input$df, {
    get_df(input$df$datapath, "clean_data_full", "full")
    get_df(input$df$datapath, "clean_data_num", "num")
    ...
    updateSliderInput(session, "threshold",
                      ...
                      max = round(max(mydf()$distance), 4) + 0.01)
  })
  
  observe({
    minval = round(min(mydf()$distance), 4)
    maxval = round(max(mydf()$distance), 4)
  ...
  output$number_name_plot = renderPlot({
    plot_linker(mydf_num(), input$threshold[1], input$threshold[2],
                input$protein)
  })
}
